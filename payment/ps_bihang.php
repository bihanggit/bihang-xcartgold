...
require './auth.php';	
require $xcart_dir.'/modules/Bihang/bihang.php';

$module_params = func_get_pm_params('ps_bihang.php');
...
if ($client->checkCallback()) {
	...
	// fetch session
	$skey = $orderids = $bihang_order->custom;
	...
	// update order status
	if ($bihang_order->status == 'completed')
	{
		$bill_output['sessid'] = func_query_first_cell("SELECT sessid FROM $sql_tbl[cc_pp3_data] WHERE ref='".$orderids."'");
	...
	$params = array(
		...
		'callback_url' => $current_location.'/payment/ps_bihang.php',
